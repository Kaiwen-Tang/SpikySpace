...
from .augment import aug_jitter_scale_pair, aug_time_warp_pair, aug_fft_phase_pair, batch_mixup, aug_pair_flip, aug_pair_random_crop
...
class BaseRunner(nn.Module):
    ...
    def fit(
        ...
        for epoch in range(start_epoch, self.max_epoches):
            ...
            pbar = tqdm(loader, total=len(loader), desc=f"Epoch {epoch+1}/{self.max_epoches}", dynamic_ncols=True, leave=False)
            for data, label in pbar:
                ...
                train_loss.update(loss_val, np.prod(label.shape))
            ...
            # print(self.network.layers[0].mixer.lif_in_proj.v_threshold.item())
            # post epoch
            ...
            if validset is not None:
                ...
                if es == EarlyStopStatus.BEST:
                    ...
                    torch.save(
                        self.best_params, f"{self.output_dir}/model_best.pkl"
                    )
                    torch.save(
                        self.best_network_params,
                        f"{self.output_dir}/network_best.pkl",
                    )
        ...
        if testset is not None:
            ...
            testset.freeup()
        torch.save(self.best_params, f"{self.output_dir}/model_best.pkl")
        torch.save(self.best_network_params, f"{self.output_dir}/network_best.pkl")
        with open(f"{self.output_dir}/res.json", "w") as f:
    ...
    def _checkpoint(self, cur_epoch, best_res, checkpoint_dir=None):
        torch.save(
            ...
            self.output_dir / "resume.pth"
            if self.output_dir is None
            else self.output_dir / "resume.pth",
        )
        print(
            f"Checkpoint saved to "
            f"{self.output_dir / 'resume.pth' if self.output_dir is None else self.output_dir / 'resume.pth'}",
            __name__,
        )
    ...
    def evaluate(self, validset: Dataset, epoch: Optional[int] = None) -> dict:
        ...
        validset.load()
        iterator = loader
        desc = f"Eval (epoch {epoch})" if epoch is not None else "Testing"
        ...
        with torch.no_grad():
            for _, (data, label) in enumerate(iterator):
                ...
                iterator.set_postfix(loss_avg=f"{eval_loss.performance():.4f}")
    ...
    def calibrate(
        ...
        """
        对网络做一次“校准前向”，用少量样本统计分布，确定逐层 clamp / quantize。
        需要各子模块（如 MambaBlockTS）实现:
            - begin_calibration(): 开始采样
            - end_calibration(q_high, rel_mse_target): 结束并固化参数
        """
        # 1) 构建校准 DataLoader（用训练集的一小部分即可）
        calibset.load()
        loader = DataLoader(
            calibset,
            batch_size=batch_size or min(self.batch_size, 64),
            shuffle=False,
            pin_memory=True,
        ...
        for m in self.network.modules():
            if hasattr(m, "begin_calibration") and hasattr(m, "end_calibration"):
                calibratables.append(m)

        if print_summary:
            printt(f"[Calibrate] Found {len(calibratables)} calibratable modules.")

        if len(calibratables) == 0:
            printt("[Calibrate] WARNING: No modules expose begin_calibration/end_calibration. "
                   "Please add them to modules like MambaBlockTS.")
            return

        # 3) 进入 eval 模式，开启采样
        self.eval()
        for m in calibratables:
            m.begin_calibration()

        # 4) 前向 num_batches 次，收集样本（不反传）
        iters = 0
        with torch.no_grad():
            for data, _ in loader:
                if use_cuda():
                    data = to_torch(data, device="cuda")
                _ = self(data)   # 前向触发各模块的采样逻辑
                iters += 1
                if iters >= num_batches:
                    break

        # 5) 结束采样并固化 clamp/quantize
        summary = []
        for idx, m in enumerate(calibratables):
            # 允许 end_calibration 返回额外信息；若无返回也没关系
            try:
                ret = m.end_calibration(q_high=q_high, rel_mse_target=rel_mse_target)
            except TypeError:
                # 兼容旧签名：m.end_calibration()
                ret = m.end_calibration()

            row = {
                "module": f"{m.__class__.__name__}#{idx}",
                "clamp": getattr(m, "clamp", None),
                "quantize": getattr(m, "quantize", None),
                "extra": ret if ret is not None else {}
            }
            summary.append(row)

        # 6) 打印/保存结果
        if print_summary:
            printt("[Calibrate] Finished. Selected clamp/quantize per-module:")
            for row in summary:
                printt(f"  - {row['module']}: clamp={row['clamp']}, quantize={row['quantize']}")

        if save_to_file and self.checkpoint_dir is not None:
            try:
                path = self.checkpoint_dir / "calibration_summary.json"
                with open(path, "w") as f:
                    json.dump(summary, f, indent=2)
                printt(f"[Calibrate] Summary saved to {path}")
            except Exception as e:
                printt(f"[Calibrate] Save summary failed: {e}")
    ...
    def predict(self, dataset: Dataset, name: str):
        ...
        prediction.to_pickle(self.output_dir / (name + "_pre.pkl"))
        return prediction
