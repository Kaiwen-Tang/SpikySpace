...
def single_huber(y_true, y_pred, delta=1.0):
    """
    单样本 Huber：对最后得到的一维向量取均值；Torch 路径对 y_true 的 NaN 做掩码，风格与 single_mse 保持一致
    """
    if isinstance(y_true, np.ndarray):
        err = y_true - y_pred
        abs_err = np.abs(err)
        ...
        loss = np.where(abs_err <= delta, quad, lin)
        return np.nanmean(loss)
    ...
    err = y_true - y_pred
    abs_err = err.abs()
    loss = torch.where(abs_err <= delta, 0.5 * err**2,
                       delta * (abs_err - 0.5 * delta))
    return loss.mean()


# 简洁别名（和 get_loss_fn 对应）
def huber(y_true, y_pred):
    return single_huber(y_true, y_pred)
