...
class MambaBlockTS(nn.Module):
    ...
    def forward(self, x):
        ...
        x_proj_part, alphaxproj, biasxproj = act_quant_fn_snn(x_proj_part, self.clip_c, 1, quant_method='elastic',
                                       symmetric=False, layerwise=True)
        x_proj_part = (x_proj_part + biasxproj/1)* alphaxproj
        x_bt = rearrange(x_proj_part, 'b l d bit -> (b bit) d l')          # (B*16, D, L)
        y_bt = F.conv1d(
            x_bt,
            weight=self.conv1d.weight,    # 共享同一组权重
        ...
        y = rearrange(y_bt, '(b bit) d l -> b l d bit', b=x_proj_part.size(0), bit=1)         # (B, L, D, 16)
        ...
        # 使用 SiLU 激活残差部分
        y, alphay, biasy = act_quant_fn_snn(y, self.clip_y, 1, quant_method='elastic',
        ...
        y = ((y-0/1) * res.unsqueeze(-1)).sum(dim=-1) * alphay
        # y = y * res
    ...
    def ssm(self, x):
        ...
        D = self.D.float()
        # ---------- step 4/6 -----------
        x, alphaxc, biasxc = act_quant_fn_snn(x, self.clip_s, 1, quant_method='elastic',
                                       symmetric=False, layerwise=True)
        x0 = x + biasxc/1
        ...
        # ----------- step 2/6 -----------
        delta, alphad, biasd = act_quant_fn_snn(delta, self.clip_delta, 1, quant_method='elastic',
                                       symmetric=False, layerwise=True)
        delta = delta + biasd/1
    ...
    def selective_scan(self, u, delta, A, B, C, D, biasxc, alphaxc):
        ...
        delta, alphad1, biasd1 = act_quant_fn_snn(delta, self.clip_delta1, 1, quant_method='elastic',
                                symmetric=False, layerwise=True)
        ...
        for i in range(l):
            if x0 is None:
                ...
                x = (deltaA[:, i].unsqueeze(-1) * (x0+biasx / 1)).sum(dim = -1) * alphax + deltaB_u[:, i]
            # ----------- step 3/6 -----------
            x, alphax, biasx = act_quant_fn_snn(x, self.clip_x, 1, quant_method='elastic',
                                       symmetric=False, layerwise=True,fifth_layer=True)
            term1 = torch.einsum('bdnt,bn->bdt', x,  1 * C[:, i, :]).sum(dim=-1)     # [B, D_in]
